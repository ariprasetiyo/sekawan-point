...
1. gradle build
   ...
   2. exec on terminal
      ...
      3. exec with exporter logging
         ...
         ```
         ```
         -XX:+UseStringDeduplication is a JVM optimization flag that reduces memory usage by automatically deduplicating identical String objects in the heap. Don't use this in ultra low micromilisecond system and memory less than 512M
         -XX:+UseG1GC use old JVM
         -XX:+UseG1GC java 21++
         -Dotel.traces.exporter=logging \
         ```
### Sample curl
...
- 1000 users concurrently in 1 minutes : 1756 RPS
  ...
  - command : k6 run stress_test_vertx_rxJava3_repository_observable.js
...
- ab script  ab -n 1000 -c 250 http://localhost:8080/test/vertx/virtualThread/eventBus
   ...
   100%  57568 (longest request)
      ```
- ab -n 1000 -c 250 http://localhost:8080/test/vertx/virtualThread/executorService
     with internal loop 2000000000
   ```
   ...
   ```
-  ab -n 1000 -c 250 http://localhost:8080/test/vertx/virtualThread/executeBlocking
    with internal loop 2000000000
      ```
      ...
      ```
-  ab -n 1000 -c 250 http://localhost:8080/test/vertx/rxJava3/organic
   with internal loop 2000000000
   ```
   ...
   ```
  - thread logs with internal loop 2000000000
     ```
     ...
     ```
- ab -n 4 -c 2 http://localhost:8080/test/vertx/virtualThread/organic/repository
   with internal loop 2000000000